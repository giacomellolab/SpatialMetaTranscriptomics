...
#### DRAFTS
  ...
  plot(log(spatial_data$Nitrospina+1), log(spatial_data$Bacillus+1))
  model = lm(log(Nitrospina+1) ~ log(Bacillus+1), data=spatial_data)
  ...
  # Create a regular grid of 1/8-mile-square crime areas via a raster
  library(raster)
  pixelsize = 3 # size of the nighbouhod? -> seems so by the plot(getisgrid)
  box = round(extent(spatial_data) / pixelsize) * pixelsize
  template = raster(box, crs = spatial_data,
                    nrows = (box@ymax - box@ymin) / pixelsize, 
                    ncols = (box@xmax - box@xmin) / pixelsize)
  
  spatial_data$PRESENT = 1
  getisraster = rasterize(spatial_data, template, field = 'Nitrospina', fun = sum)
  getisgrid = rasterToPolygons(getisraster)
  plot(getisgrid)
  # Create the list of neighbors
  neighbors = poly2nb(getisgrid)
  ...
  legend("bottomleft", inset=.02, title="Z score (local G analysis (Getis-Ord GI*))",
         c("<-1.96","-1","1","1.96",">1.96"), fill=c("#0000FF80", "#8080FF80", "#FFFFFF80", "#FF808080", "#FF000080"), horiz=TRUE, cex=0.8)
  #plot(spatial_data, border="gray", add=T)
  
  # for Methylobacterium
  getisraster1 = rasterize(spatial_data, template, field = 'Methylobacterium', fun = sum)
  getisgrid1 = rasterToPolygons(getisraster1)
  plot(getisgrid1)
  # Create the list of neighbors
  neighbors1 = poly2nb(getisgrid1)
  weighted_neighbors1 = nb2listw(neighbors1, zero.policy=T)
  
  # Perform the local G analysis (Getis-Ord GI*)
  getisgrid1$HOTSPOT = as.vector(localG(getisgrid1$layer, weighted_neighbors1))
  
  test.df=data.frame(Nitrospina_G=getisgrid$HOTSPOT,Methylobacterium_G=getisgrid1$HOTSPOT)
  
  # Color the grid cells based on the z-score
  
  breaks = c(-20, -1.96, -1, 1, 1.96, 20)
  palette=c("#0000FF80", "#8080FF80", "#FFFFFF80", "#FF808080", "#FF000080")
  col = palette[cut(getisgrid1$HOTSPOT, breaks)]
  
  # Plot
  # spplot(getisgrid)
  plot.new()
  plot(getisgrid1, col=col)
  legend("bottomleft", inset=.02, title="Z score (local G analysis (Getis-Ord GI*))",
         c("<-1.96","-1","1","1.96",">1.96"), fill=c("#0000FF80", "#8080FF80", "#FFFFFF80", "#FF808080", "#FF000080"), horiz=TRUE, cex=0.8)
  # plot(spatial_data, border="gray", add=T)
  
  View(test.df[test.df$Nitrospina_G>1.96&test.df$Methylobacterium_G>1.96,])
  
  message("== overlaping hotspots")
  sum(test.df$Nitrospina_G>1.96&test.df$Methylobacterium_G>1.96)
  message("== only Nitrospina hotspots")
  sum(test.df$Nitrospina_G>1.96&test.df$Methylobacterium_G<1.96)
  message("== only Methylobacterium hotspots")
  sum(test.df$Nitrospina_G<1.96&test.df$Methylobacterium_G>1.96)
...
### spatiol points can have atttributes
...
# feature of each point -> belings to group A-Z
...
str(spdf)
data(meuse) # data frame with X,Y as coord and fesatures -> in our casw we would have location and the abundance of each genus for example
str(meuse)
